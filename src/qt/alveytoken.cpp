...
ALVEYToken::ALVEYToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ALVEYToken),
    m_model(0),
    m_clientModel(0),
    m_tokenTransactionView(0)
{
    ...
    connect(m_tokenList, &TokenListWidget::sendToken, this, &ALVEYToken::on_sendToken);
    connect(m_tokenList, &TokenListWidget::receiveToken, this, &ALVEYToken::on_receiveToken);
    connect(m_tokenList, &TokenListWidget::addToken, this, &ALVEYToken::on_addToken);
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &ALVEYToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &ALVEYToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &ALVEYToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &ALVEYToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &ALVEYToken::removeToken);

    connect(m_tokenList, &TokenListWidget::customContextMenuRequested, this, &ALVEYToken::contextualMenu);

    connect(m_sendTokenPage, &SendTokenPage::message, this, &ALVEYToken::message);
}
...
void ALVEYToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    {
        // Set current token
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::dataChanged, this, &ALVEYToken::on_dataChanged);
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::rowsInserted, this, &ALVEYToken::on_rowsInserted);
...
void ALVEYToken::setClientModel(ClientModel *_clientModel)
{
...
void ALVEYToken::on_goToSendTokenPage()
{
    m_sendTokenPage->show();
}

void ALVEYToken::on_goToReceiveTokenPage()
{
    m_receiveTokenPage->show();
}

void ALVEYToken::on_goToAddTokenPage()
{
    m_addTokenPage->show();
}

void ALVEYToken::on_currentTokenChanged(QModelIndex index)
{
...
void ALVEYToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void ALVEYToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void ALVEYToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void ALVEYToken::contextualMenu(const QPoint &point)
{
...
void ALVEYToken::copyTokenAddress()
{
...
void ALVEYToken::copyTokenBalance()
{
    if(indexMenu.isValid())
    {
...
void ALVEYToken::copySenderAddress()
{
    if(indexMenu.isValid())
    {
...
void ALVEYToken::on_sendToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToSendTokenPage();
}

void ALVEYToken::on_receiveToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToReceiveTokenPage();
}

void ALVEYToken::on_addToken()
{
    on_goToAddTokenPage();
}
